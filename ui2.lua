...
local transparency = {
	Background = 0.15,      -- พื้นหลังหลัก
	Glow = 0.3,            -- เงา
	Accent = 0.15,         -- แถบบน
	Element = 0.2,         -- ปุ่ม/องค์ประกอบต่างๆ
	Contrast = 0.25        -- พื้นหลังส่วนย่อย
}
...
do
	...
	function utility:Pop(object, shrink)
		...
		coroutine.wrap(function()
			...
			object.ImageTransparency = transparency.Element or 0
			clone:Destroy()
		end)()
...
do
	...
	function library.new(data)
		...
		local container = utility:Create("ScreenGui", {
			...
			utility:Create("ImageLabel", {
				...
				utility:Create("ImageLabel", {
					...
					ImageTransparency = transparency.Glow,
					ScaleType = Enum.ScaleType.Slice,
				...
				utility:Create("ImageLabel", {
					...
					ImageTransparency = transparency.Contrast,
					ScaleType = Enum.ScaleType.Slice,
					SliceCenter = Rect.new(4, 4, 296, 296)
				}, {
				...
				utility:Create("ImageLabel", {
					...
					ImageTransparency = transparency.Accent,
					ScaleType = Enum.ScaleType.Slice,
					SliceCenter = Rect.new(4, 4, 296, 296)
				}, {
		...
		do
			...
			local toggleBtn = utility:Create("ImageButton", {
				...
				Position = UDim2.new(0, 500, 0.5, 0),   -- ซ้ายกลาง ย้ายได้
	...
	function section:addTextbox(data)
		...
		tInput:GetPropertyChangedSignal("Text"):Connect(function()
			...
			if button.ImageTransparency == transparency.Element and (button.Size == UDim2.new(0, 200, 0, 16) or button.Size == UDim2.new(0, 100, 0, 16)) then
				utility:Pop(button, 10)
			end
	...
	function section:addKeybind(data)
		...
		local animate = function()
			if button.ImageTransparency == transparency.Element then
				utility:Pop(button, 10)
			end
		end
	...
	function library:SelectPage(data)
		...
		if toggle then
			...
			for i = 1, #selectedPage.sections do
				...
				pageSection.container.Parent.ImageTransparency = transparency.Contrast
			end
			...
			if sectionsRequired > 0 then
				for i = existingSections + 1, #selectedPage.sections do
					...
					utility:Tween(pageSection, {ImageTransparency = transparency.Contrast}, 0.05)
				end
			end
	...
	function section:updateSlider(module)
		...
		if options.value ~= options.lvalue and slider.ImageTransparency == transparency.Element then
